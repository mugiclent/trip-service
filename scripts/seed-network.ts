/**
 * Seeds a credible Rwandan intercity network for trip-service:
 *   stops (with coordinates) → buses → routes with ordered intermediate stops →
 *   fares (from the RURA 2026 tariff) → random scheduled future trips.
 *
 * Operator: Volcano Express (org synced from user-service). Idempotent: wipes
 * the network tables and reseeds. Run: `npx tsx scripts/seed-network.ts`.
 */
import 'dotenv/config';
import { randomUUID } from 'node:crypto';
import { prisma } from '../src/models/index.js';

// Volcano Express — same id as the user-service org (so tickets/wallet line up).
const ORG = {
...
const ROUTES: { name: string; stops: string[] }[] = [
...
async function main(): Promise<void> {
  console.log('Wiping network tables…');
  ...
  // Operator (org may already be synced from user-service; upsert to be safe).
  ...
  console.log(`Seeding ${Object.keys(STOPS).length} stops…`);
  const stopId: Record<string, string> = {};
  for (const [name, [lat, lng]] of Object.entries(STOPS)) {
    const id = randomUUID();
    await prisma.stop.create({ data: { id, name, city: name, lat, lng } });
    stopId[name] = id;
  }

  console.log(`Seeding ${BUSES.length} buses…`);
  const busIds: string[] = [];
  for (const b of BUSES) {
    const bus = await prisma.bus.create({ data: { id: randomUUID(), org_id: ORG.id, ...b } });
    busIds.push(bus.id);
  }

  console.log(`Seeding ${ROUTES.length} routes + ordered stops…`);
  const routeMeta: { id: string; durationMin: number }[] = [];
  for (const r of ROUTES) {
    const id = randomUUID();
    await prisma.route.create({
      data: {
        id, org_id: ORG.id, name: r.name, slug: slugify(r.name),
        origin_stop_id: stopId[r.stops[0]], destination_stop_id: stopId[r.stops[r.stops.length - 1]],
      },
    });
    let km = 0;
    for (let i = 0; i < r.stops.length; i++) {
      await prisma.routeStop.create({ data: { id: randomUUID(), route_id: id, stop_id: stopId[r.stops[i]], order: i + 1 } });
      if (i > 0) km += haversineKm(STOPS[r.stops[i - 1]], STOPS[r.stops[i]]);
    }
    routeMeta.push({ id, durationMin: Math.round((km / 45) * 60) + 15 }); // ~45 km/h + dwell
  }

  console.log(`Seeding ${PRICES.length * 2} fares (both directions)…`);
  for (const [a, b, amount] of PRICES) {
    if (!stopId[a] || !stopId[b]) { console.warn('  missing stop for price', a, b); continue; }
    for (const [x, y] of [[a, b], [b, a]]) {
      await prisma.price.create({ data: { id: randomUUID(), boarding_stop_id: stopId[x], alighting_stop_id: stopId[y], amount } });
    }
  }
  ...
  let tripCount = 0;
  let busIdx = 0;
  for (const rm of routeMeta) {
    for (let day = 1; day <= 7; day++) {
      for (const h of hours) {
        const dep = new Date();
        dep.setDate(dep.getDate() + day);
        dep.setHours(h, 0, 0, 0);
        const bus = busIds[busIdx++ % busIds.length];
        const seats = BUSES[busIds.indexOf(bus)].total_seats;
        const arr = new Date(dep.getTime() + rm.durationMin * 60_000);
        await prisma.trip.create({
          data: {
            id: randomUUID(), org_id: ORG.id, route_id: rm.id, bus_id: bus,
            departure_at: dep, arrival_at: arr, duration_minutes: rm.durationMin,
            total_seats: seats, available_seats: seats, status: 'scheduled',
  ...
  console.log(`✅ Done: ${Object.keys(STOPS).length} stops, ${BUSES.length} buses, ${ROUTES.length} routes, ${PRICES.length * 2} fares, ${tripCount} trips.`);
  void rand;
